...
class ResNet(nn.Module):
    ...
    def forward(self, imgs, motions, mode):
        ...
        img_size = imgs.size()  # torch.Size([8, 8, 3, 256, 128])
        motion_size = motions.size()
        batch_sz = img_size[0]  # 8
        ...
        if self.cut_at_pooling:
            return x  # torch.Size([64, 2048, 8, 4])

        x = F.avg_pool2d(x, x.size()[2:])  # torch.Size([64, 2048, 1, 1])
        x = x.view(x.size(0), -1)  # torch.Size([64, 2048])

        if mode == 'cnn_rnn':
            raw = x.view(batch_sz, seq_len, -1)  # torch.Size([8, 8, 2048])

        if self.has_embedding:
            x = self.feat(x)
            x = self.feat_bn(x)  # torch.Size([64, 128])
        ...
        if mode == 'cnn_rnn':
            ...
            x = x.view(batch_sz, seq_len, -1)  # torch.Size([8, 8, 128])
            return x, raw
        elif mode == 'cnn':
            # x = x / x.norm(2, 1).expand_as(x)
            x = x / x.norm(2, 1).unsqueeze(1).expand_as(x)
